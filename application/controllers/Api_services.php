...
class Api_services extends CI_Controller {
    ...
    function execute_service($service_id) {
        ...
        $payment = $this->api_service_model->createPayment($reqData);
        ...
        if (!$billInfo['success']):
            $data = array(
                "success" => false,
                "msg" => "No bill information found"
            );
            my_json_output($data);
        endif;

        $updateData = array(
            "bill_response" => $billInfo["data"],
        );

        $this->api_service_model->updatePayment($updateData, $payment["payment_id"]);

        $billData = json_decode($billInfo["data"], true);

        if ($billData["status"] == "error") {
            $data = array(
                "success" => false,
                "msg" => $billData["message"]
            );
            my_json_output($data);
        }
        ...
        $trnData = array(
            "skyId" => 10,
            "fromAcc" => "010412200083061",
            "toAcc" => $billData["bill_account"],
            "amount" => "",
            "narration" => "Utility bill payment" . $service_id
        );

        $transactionInfo = $this->ft_model->fund_transfer($trnData);
    ...
    function top_up_fieldparams(&$fieldparams) {
        ...
        $fieldparams['sender_id'] = "01711519594";
        $fieldparams['priority'] = 1;
        $fieldparams['success_url'] = "https://mail.google.com/mail/u/0/#inbox/FMfcgxvzKQmmrccRdmRbzLHcjLcWPvSF";
        $fieldparams['failure_url'] = "https://mail.google.com/mail/u/0/#inbox/FMfcgxvzKQmmrccRdmRbzLHcjLcWPvSF";
        $fieldparams['calling_method'] = "get";
    }
    
     function wasa_fieldparams(&$fieldparams) {
        $fieldparams['utility_name'] = "WASA";
        $fieldparams['stockid'] = "";
    }
