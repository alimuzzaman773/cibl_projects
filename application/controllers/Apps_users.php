...
class Apps_users extends MX_Controller {
    ...
    function register()
    {
        $data['body_template'] = 'apps_users/register.php';
    ...
    function get_customer_info()
    {
        ...
        $cbsData = array(
            "account_no" => getAccountNo($account_number),
            "branch_id" => getBranchId($account_number)
        );

        $accountInfo = $this->cbs_services->getAccountInfo($cbsData);
        if (!$accountInfo["success"]) {
            $json = array(
                "success" => false,
                "msg" => "Server process error",
                "log" => $accountInfo
            );
            my_json_output($json);
        }

        $accountInfo = json_decode($accountInfo['data'], true);
        $accountInfo = $accountInfo[0];


        if (isset($accountInfo["ERROR_CODE"])) {
            $json = array(
                "success" => false,
    ...
    function confirm_registration()
    {
        ...
        $uInfo = $this->db->where("cfId", $accountInfo['CUSTOMER_ID'])
                          ->get('apps_users_mc');
        if($uInfo->num_rows() > 0):
            $json = array(
                "success" => false,
                "msg" => "Account is already registered with us",
                "log" => $accountInfo
            );
            my_json_output($json);
        endif;
        ...
        $userInfo['isActive'] = 0;
        $userInfo['isReset'] = 1;
        $userInfo['cfId'] = $accountInfo['CUSTOMER_ID'];

        /* $dob = new DateTime($serviceResult["CUSTPERSONAL"]["DOB"]);
          if ($dob):
          $userInfo['dob'] = $dob->format("Y-m-d");
          endif;
         */

        //$serviceResult = $accountInfo;
        $userInfo['dob'] = @$accountInfo["BIRTH_DATE"];
        $userInfo['gender'] = @$accountInfo["SEX"];
        $userInfo['fatherName'] = @$accountInfo["FATHERS_NM"];
        $userInfo['motherName'] = @$accountInfo["MOTHERS_NM"];
        $userInfo['userMobNo1'] = @$accountInfo["PHONE"];
        $userInfo['userMobNo2'] = @$accountInfo["MOBILE2"];
        $userInfo['userName'] = @$accountInfo["ACC_TITLE"];
        $userInfo['userEmail'] = @$accountInfo["EMAIL"];
        $userInfo['currAddress'] = @$accountInfo["ADDRESS"];
        $userInfo['parmAddress'] = @$accountInfo["ADDRESS"];
        $userInfo['billingAddress'] = @$accountInfo["ADDRESS"];
        $userInfo['homeBranchCode'] = @getBranchId($account_number);
        
        $summaryData = array(
            "customer_id" => $accountInfo['CUSTOMER_ID']
        );

        $summaryInfo = $this->cbs_services->getAccountSummary($summaryData);
        ...
        $this->load->model("apps_users_model");
        $userInfo = $this->apps_users_model->getAccountSerial($userInfo, $accountListInfo);
        ...
        if($userInfo['success']):
            ...
            if($otp_channel == 'email'):
                $edata = array(
                    ...
                    'from' => 'Premiere Banking',
                    'subject' => 'Premeire Banking APP Account Creation',
                    'body' => 'Dear Sir, we have created an account for you in Premiere Banking APP, please contanct our call center and activate it after 24 Hours'
                );
                $otpResult = $this->mailer_model->sendMail($edata);
            else: 
                $this->load->library('sms_service');
                $edata = array(
                    'mobileNo' => $accountInfo['PHONE'],
                    'from' => 'Premiere Banking',
                    'subject' => 'Premeire Banking APP Account Creation',
                    'message' => 'Dear Sir, we have created an account for you in Premiere Banking APP, please contanct our call center and activate it after 24 Hours'
                );
                $otpResult = $this->sms_service->smsService($edata);
            endif;
        endif;
        
        $userInfo['messageResponse'] = $otpResult;
        my_json_output($userInfo);
    }
