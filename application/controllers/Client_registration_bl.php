...
class Client_registration extends CI_Controller {
    ...
    {        
        ...
        $this->load->model(array("au_model"));
        $skyRes = $this->au_model->getUserById($skyId);
        if(empty($skyRes)):
        ...
        $userGroup = $this->au_model->getAppsUserGroup(array('isActive' => 1));
        if(count($userGroup)):
    ...
    function save_limit_package()
    {
        ...
        try {
            ...
            $revisionData = $this->revision_model->getAppsUser($skyId);
            if(!$revisionData):
                throw new Exception('No sky info found for revision');
            endif;
            
            $revisionUserData = $revisionData->row();
            $revisionUserData->appsGroupId = $appsGroupId;
            
            $appsUserGroup = $this->au_model->getGroupById($appsGroupId);
            if(empty($appsGroupResult)):
                throw new Exception('No apps user group found');
            endif;
            
            $appsUserGroup = (object)$appsUserGroup;
            ...
            foreach ($groupFields as $k => $v):
                $revisionUserData->{$v} = $appsUserGroup->{$v};
            endforeach;
