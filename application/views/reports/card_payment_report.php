<h2 class="center">Card Payment Report</h2>
<form name="myform" id="myform" method="get" target="_blank" action="<?=$base_url.'ajax_report/card_payment_report/'?>">
    ...
    <tr>
        ...
        <td>
            <div class="col-sm-3 col-xs-12">
                <select class="form-control input-sm" name="billerId" id="billerId">
                    <option value="">All</option>
                    <?php foreach($billerList as $b): ?>
                    <?php if(in_array(strtoupper($b->billerCode), array('BC0033', 'BC0019', 'BC0023', 'BC0030'))): ?>
                    <option value="<?=$b->billerId?>"><?=$b->billerName?></option>
                    <?php endif; ?>
                    <?php endforeach; ?>
                </select>
            </div>
...
<script type="text/javascript">
    ...
    var generateReport = function(dom)
    {
        ...
        $.ajax({
            url:'<?=$base_url.'ajax_report/card_payment_report/'?>',
            type:'post',
            data:formData,
            dataType:'json',
